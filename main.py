...
class DatabaseSetup:
    # Клас для підготовки даних та налаштування бази даних SQLite
    ...
    def prepare_and_load_data(self):
        ...
        # Витягуємо числа з текстового поля зарплати за допомогою регулярних виразів
        ...
        # Якщо максимальна зарплата не вказана, беремо значення мінімальної
        df['Max Salary'] = df['Max Salary'].fillna(df['Min Salary'])

        # Працюємо з датами: перетворюємо формат та виділяємо рік
        df['Date Posted'] = pd.to_datetime(df['Date Posted'])
        df['Year'] = df['Date Posted'].dt.year

        # Встановлюємо з'єднання з БД і записуємо основну таблицю jobs
        self.conn = sqlite3.connect(self.db_file)
        df.to_sql('jobs', self.conn, if_exists='replace', index=False)

        # Створюємо окрему таблицю для навичок, розбиваючи їх по комі
        ...
        skills_df['Skill'] = skills_df['Skill'].str.strip() # Прибираємо зайві пробіли
        skills_df.to_sql('job_skills', self.conn, if_exists='replace', index=False)

        return self.conn
    
    def close_connection(self):
        # Закриваємо з'єднання з базою даних, якщо воно відкрите
        if self.conn:
            self.conn.close()

class BasicQueries:
    # Клас для простих SQL-запитів
    def __init__(self, conn):
        self.conn = conn

    def get_first_10_jobs(self):
        # Отримуємо перші 10 записів
    ...
    def get_jobs_with_sql_skill(self):
        # Використовуємо LIKE для пошуку вакансій, де вимагається SQL
    ...
    def get_unique_locations_and_companies(self):
        # DISTINCT допомагає отримати лише унікальні пари локації та компанії
        query = "SELECT DISTINCT Location, Company FROM jobs"
        print(f"Унікальні Location та Company:\n{pd.read_sql_query(query, self.conn)}\n")

class AnalyticalQueries:
    # Клас для базової аналітики та агрегатних функцій
    def __init__(self, conn):
        self.conn = conn

    def run_task_3(self):
        # Рахуємо середню мінімальну зарплату за рівнем досвіду
        ...
        # Підраховуємо загальну кількість вакансій для кожного рівня досвіду
        ...
        # Знаходимо абсолютний мінімум та максимум зарплати у всій базі
    ...
    def run_task_4(self):
        # Рахуємо вакансії з високою зарплатою (більше 50к) по кожній індустрії
        ...
        # Обчислюємо середню зарплату по індустріях
...
class ComplexQueries:
    # Клас для складніших зрізів та групувань
    def __init__(self, conn):
        self.conn = conn

    def run_task_5(self):
        # Групуємо відразу за двома параметрами: містом та досвідом
        ...
        # Групуємо за індустрією та типом роботи (повна/часткова зайнятість тощо)
        ...
        # Аналізуємо середню зарплату в розрізі локації та досвіду
    ...
    def run_task_6(self):
        # Сортуємо дані за спаданням, щоб знайти топ-5 найдорожчих вакансій
        ...
        # Використовуємо таблицю навичок, щоб знайти найпопулярніші з них
        ...
        # Знаходимо компанії, які були найактивнішими у 2023 році
